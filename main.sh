...
./app/main_part_from_image_list.sh "$TMP_MAIN_PART_MOVIE_FILE_PATH" "$TMP_IMAGE_LIST_FILE_PATH" "$BACKGROUND_IMAGE_FILE_PATH" x "$TMP_RT_FILE_PATH" x x
...
./app/subtitle_movie.sh "$TMP_SUBTITLE_MOVIE_FILE_PATH" "$TMP_CONCAT_MOVIE_FILE_PATH" "$TMP_SUBTITLE_FILE_PATH" "$FONT_FILE_PATH"
