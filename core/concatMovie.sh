...
# 同じ条件でエンコードし直す
ffmpeg -y -i $TITLE_MOVIE_FILE_PATH -r 60 -s 1920x1080 -c:v h264_nvenc -b:v 2M -preset fast $REENCODED_TITLE_MOVIE_FILE_PATH

ffmpeg -y -i $MAIN_PART_MOVIE_FILE_PATH -r 60 -s 1920x1080 -c:v h264_nvenc -b:v 2M -preset fast $REENCODED_MAIN_PART_MOVIE_FILE_PATH
ffmpeg -y -i $LOGO_ROTATE_MOVIE_FILE_PATH -r 60 -s 1920x1080 -c:v h264_nvenc -b:v 2M -preset fast $REENCODED_LOGO_ROTATE_MOVIE_FILE_PATH

# タイトル動画、ロゴ回転動画、メインパート動画を結合する
ffmpeg -y -i $REENCODED_TITLE_MOVIE_FILE_PATH -i $REENCODED_MAIN_PART_MOVIE_FILE_PATH -i $REENCODED_LOGO_ROTATE_MOVIE_FILE_PATH -filter_complex "[0:v:0][1:v:0][2:v:0]concat=n=3:v=1:a=0[v]" -map "[v]" -c:v h264_nvenc -b:v 2M -preset fast $CONCAT_MOVIE_FILE_PATH
