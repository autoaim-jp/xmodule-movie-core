...
FONT_FILE_PATH=${3:-"${SCRIPT_DIR_PATH}../asset/SourceHanSans-Bold.otf"}
SUBTITLE_MODE=${4:-"bottom"}
MOVIE_PART_LIST_FILE_PATH=${5}
SOUND_FILE_PATH=${6}
...
# 動画一覧と音声を結合し、字幕をつける
ffmpeg -y -f concat -safe 0 -i "$MOVIE_PART_LIST_FILE_PATH" -i "$SOUND_FILE_PATH" \
-vf "$filter_text" -r 60 -s 1920x1080 -c:v h264_nvenc -b:v 4M -maxrate 6M -bufsize 8M \
-preset slow -profile:v high -rc-lookahead 32 -preset fast -c:a aac -strict experimental "$SUBTITLE_MOVIE_FILE_PATH"
