...
cat $CENTER_IMAGE_LIST_FILE_PATH | while IFS=',' read -r part_sec fade_in_sec fade_out_sec file_path; do
    ...
    ffmpeg -y -loop 1 -i $file_path -vf "scale=1422:800,format=rgba,fade=t=in:st=0:d=${fade_in_sec}:alpha=1,fade=t=out:st=${fade_out_start}:d=${fade_out_sec}:alpha=1" -t ${part_sec} -c:v qtrle $FADE_MOVIE_PATH < /dev/null
