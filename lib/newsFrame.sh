...
left_bottom_image_height=200          # 左下画像の高さ

# 背景が透明で、フェードイン、フェードアウトの動画パーツ
ffmpeg -y -loop 1 -i $center_image -vf "scale=1200:800,format=rgba,fade=t=in:st=0:d=4:alpha=1,fade=t=out:st=6:d=4:alpha=1" -t 10 -c:v qtrle image_fade.mov
...
-c:v libx264 -t 10 -pix_fmt yuv420p output_video.mp4

# 別動画の中央にフェードイン・アウトの動画をオーバーラップさせる
ffmpeg -y -i output_video.mp4 -i image_fade.mov -filter_complex "overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2:enable='between(t,0,10)'" -c:a copy output_video2.mp4


xdg-open output_video2.mp4
